-- process_sale ya calculaba benefit_available_before/after y los devolvía
-- en el JSON de respuesta, pero nunca los guardaba en la fila de la venta.
-- Sin esa persistencia, un ticket reimpreso más tarde (u otro día) no puede
-- reconstruir el "crédito antes / crédito disponible después" reales de esa
-- venta puntual. Se agregan employee_benefit_before/after al INSERT; el
-- resto de la función queda idéntico a la versión vigente.
CREATE OR REPLACE FUNCTION public.process_sale(p_client_type text, p_items jsonb, p_payment_method text, p_amount_received numeric, p_discount numeric, p_opened_by text, p_employee_id bigint DEFAULT NULL::bigint, p_employee_sale_type text DEFAULT NULL::text, p_employee_extra_payment text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
