...
-- seguridad"): process_sale recibía p_benefit_enabled/p_benefit_daily_amount/
-- p_weekly_credit_enabled/p_weekly_credit_limit como parámetros del
-- LLAMADOR (agregados en 20260822010000_process_sale_configurable_benefit_credit.sql
-- para que Ajustes pudiera configurarlos). db.js sí los toma de `settings`
-- antes de llamar al RPC, pero el RPC en sí -- SECURITY DEFINER, GRANT a
-- anon, llamable directo por REST con la anon key embebida en el
-- instalador/bundle web -- nunca los revalidaba: cualquiera podía llamar
-- process_sale directo con p_weekly_credit_limit=999999 o
-- p_benefit_daily_amount=999999 y saltarse por completo los topes de
-- beneficio/crédito de empleado configurados en Ajustes. Fraude financiero
-- directo, no solo un hueco teórico.
--
-- Corrección: los 4 parámetros se QUITAN de la firma -- process_sale ahora
-- lee `settings` él mismo (SECURITY DEFINER rodea RLS) filtrado por
-- p_branch_id, con el mismo parseo/defaults que
-- db.js::getEmployeeBenefitSettings() (benefit_enabled/weekly_credit_enabled
-- por defecto true salvo 'false' explícito; benefit_daily_amount/
-- weekly_credit_limit por defecto 100/500 salvo un valor numérico > 0
-- guardado). Cambia la lista de tipos de parámetros -> DROP FUNCTION
-- explícito antes del CREATE, no CREATE OR REPLACE (ver
-- [[postgres-create-or-replace-overload-trap]]). Resto de la función
-- IDÉNTICO a 20260822070000_folio_counters_branch_scoped.sql.
